                ...
                {registrations.map(r => (
                  <div key={r._id} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr auto auto', gap: '1rem', alignItems: 'center', padding: '1rem', background: '#080000', borderRadius: '4px', border: `1px solid ${r.status === 'approved' ? '#004411' : '#1a0000'}` }}>
                    ...
                    <div style={{ fontSize: '0.7rem' }}>
                      <a href={r.payment.screenshotPath?.startsWith('http') ? r.payment.screenshotPath : `${window.location.origin}/${r.payment.screenshotPath?.replace(/\\/g, '/')}`} target="_blank" rel="noreferrer" style={{ color: 'var(--blue)' }}>VIEW RECEIPT</a>
